...
public interface IShopRepository : IRepository {
    ...
    Task<Shop?> FindByAdminIdAsync(Guid adminId, CancellationToken cancellationToken = default);
    ...
    Task<Shop> UpdateAsync(Shop oldEntity, Shop newEntity, CancellationToken cancellationToken = default);
    Task DeleteAsync(Shop entity, CancellationToken cancellationToken = default);
}